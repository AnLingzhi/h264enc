...
reg valid_chroma_pred;

always @(posedge clk or negedge rst_n)
	if (!rst_n)
		valid_chroma_pred <= 1'b0;
	else if (chroma_num_r2 == 3'd7)
		valid_chroma_pred <= 1'b1;
	else
		valid_chroma_pred <= 1'b0;
...
always @(posedge clk or negedge rst_n) begin
	if(!rst_n) begin
		...
		else if (valid_chroma_pred && (cost_total<cost_chroma_min || (cost_total == cost_chroma_min && prev_mode < chroma_mode))) begin
			cost_chroma_min <= cost_total;	
			chroma_mode <= prev_mode; 
		end
	end
	else if (curr_state_r2==CHROMA_WAIT && valid_chroma_pred && ((cost_total<cost_chroma_min) || (cost_total == cost_chroma_min && prev_mode < chroma_mode))) begin
		cost_chroma_min <= cost_total;
		chroma_mode <= prev_mode;
	end
end
