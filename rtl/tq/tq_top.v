...
// mem if for db
input  						db_mem_rd   ;
input  [4:0]				db_mem_raddr;
output [127:0]				db_mem_rdata;
input  						db_mem_wr   ;
input  [4:0]				db_mem_waddr;
input  [127:0]				db_mem_wdata;
...
//------------------------------------------
//	Rec. Memory (two port, ping-pong buffer x2)
//  00-15: i16x16_rec.
//  16-23: chroma_rec.
//  24-31: reserved
//  Total: 32x2 = 64
//------------------------------------------
wire 		rec_0_we   , rec_0_rd   , rec_1_we   , rec_1_rd   ; 	
wire [4:0]	rec_0_waddr, rec_0_raddr, rec_1_waddr, rec_1_raddr;     
wire [127:0]rec_0_wdata, rec_0_rdata, rec_1_wdata, rec_1_rdata;  

assign rec_mem_we    = i4x4_val_r4 | p16x16_en_r4 | rec_y_en_d5 | rec_u_en_d5 | rec_v_en_d5;
assign rec_mem_waddr = i4x4_val_r4  ? {1'b0, luma_num_r4} :
					   p16x16_en_r4 ? {1'b0, luma_num_r4} :
					   rec_y_en_d5  ? {1'b0, rec_num_d5}  :
					   rec_u_en_d5  ? {3'b100, rec_num_d5[1:0]} :
					   rec_v_en_d5  ? {3'b101, rec_num_d5[1:0]} : 'b0;
assign rec_mem_wdata = {rec33, rec32, rec31, rec30, 
						rec23, rec22, rec21, rec20, 
						rec13, rec12, rec11, rec10, 
						rec03, rec02, rec01, rec00};

assign rec_0_we    = mem_sel ? rec_mem_we    : db_mem_wr    ;
assign rec_0_waddr = mem_sel ? rec_mem_waddr : db_mem_waddr ;
assign rec_0_wdata = mem_sel ? rec_mem_wdata : db_mem_wdata ;
assign rec_1_we    = mem_sel ? db_mem_wr     : rec_mem_we   ;           
assign rec_1_waddr = mem_sel ? db_mem_waddr  : rec_mem_waddr;           
assign rec_1_wdata = mem_sel ? db_mem_wdata  : rec_mem_wdata;           
           
assign rec_0_rd    = mem_sel ? 1'b0			 : db_mem_rd	;
assign rec_1_rd    = mem_sel ? db_mem_rd     : 1'b0			;
assign rec_0_raddr = db_mem_raddr;
assign rec_1_raddr = db_mem_raddr;
assign db_mem_rdata= mem_sel ? rec_1_rdata   : rec_0_rdata  ;

tq_ram_2p_128x24 u_rec_ram_128x24_0 (
				.clk     ( clk           	),  
				.we      ( rec_0_we			),				
				.waddr   ( rec_0_waddr  	),
				.wdata   ( rec_0_wdata  	),
...
tq_ram_2p_128x24 u_rec_ram_128x24_1 (
				.clk     ( clk           	),  
				.we      ( rec_1_we			),				
				.waddr   ( rec_1_waddr  	),
				.wdata   ( rec_1_wdata  	),
...
/*
wire [127:0]				rec_wdata    ;
wire 						rec_y_we     ;
wire [4:0]					rec_y_waddr  , rec_y_raddr;
wire 						rec_u_we     ;
wire [2:0]					rec_u_waddr  , rec_u_raddr;
wire 						rec_v_we     ;
wire [2:0]					rec_v_waddr  , rec_v_raddr;

assign rec_wdata =     {rec33, rec32, rec31, rec30, 
						rec23, rec22, rec21, rec20, 
						rec13, rec12, rec11, rec10, 
						rec03, rec02, rec01, rec00};

assign rec_y_waddr = i4x4_val_r4  ? {mem_sel, luma_num_r4} :
					 p16x16_en_r4 ? {mem_sel, luma_num_r4} : 
					 rec_y_en_d5  ? {mem_sel, rec_num_d5}  : 'b0;
assign rec_y_we    = i4x4_val_r4 | p16x16_en_r4 | rec_y_en_d5;
assign rec_u_waddr = {mem_sel, rec_num_d5[1:0]};
assign rec_u_we    = rec_u_en_d5;
assign rec_v_waddr = {mem_sel, rec_num_d5[1:0]};
assign rec_v_we    = rec_v_en_d5;

assign rec_y_raddr  = {~mem_sel, db_y_raddr};
assign rec_u_raddr  = {~mem_sel, db_u_raddr};
assign rec_v_raddr  = {~mem_sel, db_v_raddr};


tq_ram_2p_128x32 u_rec_y_ram_128x32(         
				.clk     ( clk         ),
				.wdata   ( rec_wdata   ),
				.waddr   ( rec_y_waddr ),
				.we      ( rec_y_we    ),
...
tq_ram_2p_128x8 u_rec_u_ram_128x8(         
				.clk     ( clk         ),
				.wdata   ( rec_wdata   ),
				.waddr   ( rec_u_waddr ),
				.we      ( rec_u_we    ),
...
tq_ram_2p_128x8 u_rec_v_ram_128x8(         
				.clk     ( clk         ),
				.wdata   ( rec_wdata   ),
				.waddr   ( rec_v_waddr ),
				.we      ( rec_v_we    ),
